...
export const schema = z.object({
  identityType: z.enum(["NIN", "PASSPORT", "REFUGEE_ID", "ALIEN_ID", "DRIVER_LICENCE"]),
  identityCountry: z.string().trim().min(2).max(3).default("UG"),
  identityNumber: z.string().trim().min(5).max(32),
});
...
export async function postIdentityVerify(input: z.infer<typeof schema>, init?: RequestInit): Promise<OutputType> {
  ...
  const response = await fetch("/_api/identity-verify", {
    method: "POST",
